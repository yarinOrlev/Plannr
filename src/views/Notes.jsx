...
const Notes = () => {
  const { activeProduct, activeNotes, addNote, deleteNote } = useProductContext();
  ...
  const handleAdd = (e) => {
    ...
    addNote({ title:form.title, content:form.content, tag:form.tag });
    ...
    <div className="content-area animate-fade-in notes-layout">
      <header className="page-header">
        <div>
          ...
          <p className="text-secondary text-lg">רעיונות והחלטות עבור <strong className="text-primary">{activeProduct.name}</strong></p>
        </div>
      ...
      <div className="notes-tags" style={{ display:'flex', gap:'0.4rem', flexWrap:'wrap' }}>
        {['הכל',...TAGS].map(t => (
      ...
      {showForm && (
        <form onSubmit={handleAdd} className="glass-panel p-6 animate-fade-in">
          <h3 className="text-h3 mb-4">הערה חדשה</h3>
